...
namespace DataOperations.OData.Client
{
    public class ODataOperationsDispatcher : IOperationsDispatcher 
    {
        ...
        public async Task<T> CreateAsync<T>(T entity) where T : IBaseDTOWithIDAndETag
        {
            ...
            var CustomHeaders = _context.ValidationRequest(keyString).Result;
            return (await _context.FireRemoteRequestAsyncWithEtag<T>(entity, keyString, HttpMethod.Post, null, CustomHeaders));
        }
        public async Task DeleteAsync<T>(string ID, string eTag) where T : IBaseDTOWithIDAndETag
        {
            ...
            var CustomHeaders = _context.ValidationRequest(keyString).Result;
            await _context.FireRemoteRequestAsyncWithEtagWithNoReturnOrPayload<T>(eTag, keyString, HttpMethod.Delete, null, CustomHeaders);
        }
        ...
        public virtual async Task UpdateAsync<T>(T entity) where T : IBaseDTOWithIDAndETag
        {
            ...
            var CustomHeaders = _context.ValidationRequest(keyString).Result;
            await _context.FireRemoteRequestAsyncWithEtagWithNoReturn<T>(entity, keyString, HttpMethod.Patch, null, CustomHeaders);
        }
