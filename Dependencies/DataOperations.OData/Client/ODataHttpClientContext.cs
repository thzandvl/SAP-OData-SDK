...
namespace DataOperations.OData.Client
{
    public class ODataHttpClientContext : IClientContext
    {
        ...
        public virtual async Task<Dictionary<string, string>> ValidationRequest(string tailUri = "/")
        {
            Dictionary<string, string> CustomHeaders = new Dictionary<string, string>();
            CustomHeaders.Add("x-csrf-token", "fetch");

            HttpRequestMessage _requestMessage = await BuildHttpRequestMessageAsync("", tailUri, HttpMethod.Head, null, CustomHeaders, "");
            HttpResponseMessage _resp = await ExecuteHttpRequest(_requestMessage);

            var token = _resp.Headers.TryGetValues("x-csrf-token", out var values) ? values.FirstOrDefault() : null;
            var cookies = _resp.Headers.TryGetValues("Set-Cookie", out values) ? values : null;
            string cookieString = "";
            ...
            CustomHeaders.Add("x-csrf-token", token);
            CustomHeaders.Add("cookie", cookieString);
